...
class Game
{
    ...
    foreach ($this->getUnconqueredCountries() as $attackingCountry) {
      ...
      if ($defendingCountry) {
        ...
        if ($defendingCountry->isConquered()) {
          ...
          $attackingCountry->increaseTroops();
  ...
  /**
   * Increase number of troops at the end of the rounds
   */
  protected function increaseEndOfRoundTroops()
  {
    array_map(function (CountryInterface $country) {
      $country->increaseTroops(3);
    }, $this->getUnconqueredCountries());
  }
}
